...
/**
 * isChangeDir - checks ":" if is in the current directory.
...
int isExecutable(shellData *datash)
{
	...
	findError(datash, 127);
	return (-1);
}

/**
 * checkErrorCommand - verifies if user has permissions to access
 *
...
int executeCommand(shellData *datash)
{
	...
	if (exec == 0)
	{
		...
		if (checkErrorCommand(dir, datash) == 1)
			return (1);
	}
